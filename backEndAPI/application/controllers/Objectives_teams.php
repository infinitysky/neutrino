...
class Objectives_teams extends CI_Controller
{
    function __construct()
    {
        ...
        parent::__construct();
        $this->load->model('Objectives_teams_model');
    ...
    {
        $tempData=$this->Objectives_teams_model->get_all();
...
//    }
    ...
    {
        $row = $this->Objectives_teams_model->get_by_id($id);

        if ($row) {
            $data = array(
                'record_id' => set_value('record_id', $row->record_id),
    ...
    public function create()
    {
        ...
        if($checkArray!=0){
            $last_insert_id=$this->Objectives_teams_model->insert($checkArray);
    ...
    public function update($id,$updateData)
    {
        ...
        if ($row) {
            ...
            if($processArray!=0) {
                ...
                $affectedRowsNumber = $this->Objectives_teams_model->update($id, $data);
    ...
    public function delete($id)
    {
        ...
        if ($row) {
            $affectRow=$this->Objectives_teams_model->delete($id);
            $tempReturnArray=array(
    ...
    public function batch_create()
    {
        ...
        if (empty($Data['data'])){
            ...
            $last_query=$this->Objectives_teams_model->batch_insert($dataArray);
    ...
    public function update_team_objectives()
    {
        ...
        elseif(empty($Data['data']['team_id'])){
            ...
            if($processArray) {
                ...
                if (!empty($processArray) && $insertArraySize != 0) {
                    ...
                    $insertedNumber = $this->Objectives_teams_model->batch_insert($insertArray);

                }
                if (!empty($processArray) && $deleteArraySize != 0) {
                    // $last_query=$this->Objectives_teams_model->batch_delete_by_team_id($processArray['delete']);
                    array_push($deleteArray, 0);
                    for ($i = 0; $i < $deleteArraySize; $i++) {
                        array_push($deleteArray, $processArray['delete'][$i]);
                    }
                    $deletedNumber = $this->Objectives_teams_model->batch_delete_by_team_id($teamId, $deleteArray);
                }
    ...
    public function relationship_analysis($updateData){
        ...
        if(empty($updateData)||empty($updateData['team_id'])){
            ...
            //Get Current Members
            $objectivesArray=$this->Objectives_teams_model->get_by_team_id($updateData['team_id']);
            $Objectives=$updateData['new_objectives'];

            if ($objectivesArray){
                $arrayLength=count($objectivesArray);
            ...
            $arrayLength=count($Objectives);

            for ($i=0;$i<$arrayLength;$i++){
                array_push($newObjectivesIdArray,$Objectives[$i]);
            }
            ...
            $old=$objectivesIdArray;
            $new=$newObjectivesIdArray;
            $removed=array_diff($old,$new);
            $added=array_diff($new,$old);
            $same=array_diff($objectivesIdArray,$objectivesIdArray); //empty

            $removedObjectives=array_diff($objectivesIdArray,$newObjectivesIdArray);

            $addedObjectives=array_diff($newObjectivesIdArray,$objectivesIdArray);
            ...
            $checkedArray=array(
                "insert"=>array_values($addedObjectives),
                "delete"=>array_values($removedObjectives),
            );
        }
    ...
    public function batch_delete_by_team_id()
    {
        ...
        if (empty($Data['data'])){
            ...
            $last_query=$this->Objectives_teams_model->batch_delete_by_team_id($teamId,$deleteArray);
            $arraySize=count($dataArray);
            if($last_query==$arraySize){
                $successArray = array(
    ...
    public function get_by_objective_id($id){
        ...
        $i=0;
        $data=[];
        $row = $this->Objectives_teams_model->get_by_objective_id($id);
    ...
    public function get_by_team_id($id){
        ...
        $data=[];
        $row = $this->Objectives_teams_model->get_by_team_id($id);
        if ($row){
            $length= count($row);
            for($i=0;$i<$length;$i++){
    ...
    public function get_by_team_and_Users_details(){
        ...
        $data=[];
        $row = $this->Objectives_teams_model->get_team_and_users_details();
        if ($row){
            $length= count($row);
            for($i=0;$i<$length;$i++){
